...
def donuts(n):
    num = n if n < 10 else "muitos"
    return f"Número de donuts: {num}"
...
def fixa_primeiro(s):
    new = s.replace(s[0], "*").replace("*", s[0], 1)
    return new
...
def palindrome(s):
    n = [s[len(s)-1 - i] for i in range(len(s))]
...
def busca(frase, palavra):
    ...
    for i in range(len(frase)):
        if frase[i:i+len(palavra)] == palavra: c += 1
    return c
