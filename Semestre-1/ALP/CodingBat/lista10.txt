...
# Solução gambiarrenta, depois tento melhorar :(
...
def cat_dog(s):
    ...
    for i in range(2, len(s)):
        if s[i-2] + s[i-1] + s[i] == "cat": cat += 1
        if s[i-2] + s[i-1] + s[i] == "dog": dog += 1
    return cat == dog
...
def count_code(s):
    ...
    for i in range(3, len(s)):
      if s[i-3] + s[i-2] + s[i] == "coe": count += 1
    return count
...
def end_other(a, b):
  ...
  return a[-size_b:] == b or b[-size_a:] == a
...
def soma_na_lista(n, lista):
  for x in lista:
    for i in range(len(lista)-1):
      if x + lista[i] == n and x != lista[i]: return True
  return False
...
def fila_tijolos(n_peq, n_gra, meta):
  ...
  if p > 0 and g > 0:
    ...
    return meta <= p
# Novamente, solução gambiarrenta.
# Vai falhar com número de tijolos grandes maior que 2.
# Depois tento melhorar.
