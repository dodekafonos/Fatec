...
# for word in txtB:
#     if word[0] in "zmb" and word[-1] not in "zmb":
#         preps.append(word)
#     if len(word) == 7 and word[-1] not in "zmb":
#         verbos.append(word)
#         if word[0] not in "zmb":
#             verbospp.append(word)
# print(f"Preposições: {len(preps)}")
# print(f"Verbos: {len(verbos)}")
# print(f"Verbos em primeira pessoa: {len(verbospp)}")
usual = "ABCDEFGHIJKLMNOPQRST"
...
def abc(p):
    resultado = ""
    for letra in p:
        resultado = resultado + usual[ordem.index(letra)]
    return resultado

listaB = txtB.sort(key=abc)
...
#         n += str(ordem.index(l))
#     ordem.index(n)
#     for exp in range (0, len(str(n)-1)):
#         dec += n ** exp
